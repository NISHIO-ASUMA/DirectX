...
void InitScore(void)
{
	...
	for (nCntscore = 0; nCntscore < MAX_NUM_SCORE; nCntscore++)
	{
		...
		//頂点座標の設定
		pVtx[0].pos = D3DXVECTOR3(50.0f +nCntscore * 50.0f , 0.0f, 0.0f);
		pVtx[1].pos = D3DXVECTOR3(50.0f +nCntscore * 50.0f + 50.0f, 0.0f, 0.0f);
		pVtx[2].pos = D3DXVECTOR3(50.0f +nCntscore * 50.0f , 30.0f, 0.0f);
		pVtx[3].pos = D3DXVECTOR3(50.0f +nCntscore * 50.0f +50.0f, 30.0f, 0.0f);
...
void DrawScore(void)
{
	...
	for (nCntscore = 0; nCntscore < MAX_NUM_SCORE;nCntscore++)
	{
		if (g_aScore[nCntscore].bUse == true)
		{
			...
			//描画(1種類目)
			pDevice->DrawPrimitive(D3DPT_TRIANGLESTRIP, nCntscore * 4, 2);//1個目のプレイヤーの始点(0番目～3番目)
		}
